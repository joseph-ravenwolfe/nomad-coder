...
- **No unilateral updates.** Do not run `npm update`, `npm upgrade`, or bump any dependency version without explicit operator approval.
...
- **Exception: patch-only security fixes.** A patch version bump that fixes a known CVE and produces a clean `npm audit` result may be applied without waiting for full review (e.g. the path-to-regexp fix in #134). Document the CVE and audit result in the commit message.

When in doubt, ask the operator before touching `package.json` or `package-lock.json`.
