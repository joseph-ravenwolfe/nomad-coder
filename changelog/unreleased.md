...
- Fixed outbound name tag rendering literal backticks when no `parse_mode` is set — `buildHeader()` now returns plain text (no backtick wrapping) for the `plain` field and the default (no parse_mode) branch; backtick formatting only applies with explicit `Markdown` or `MarkdownV2` parse modes
