...
- **`set_temporary_reaction` tool** — sets a reaction that auto-reverts on the next outbound action (any send, edit, or file transfer) or after an optional `timeout_seconds` deadline; supports `restore_emoji` to snap back to a specific emoji (e.g. 👀 → 🫡) or remove the reaction entirely; eliminates the 👀→🫡 manual two-step that previously required explicit `set_reaction` calls on every voice message; implemented via a module-level slot in `temp-reaction.ts` cleared by the outbound proxy
- **`set_temporary_reaction` concept documented** — added to `docs/super-tools.md` as a planned tool; auto-reverts a reaction to a `restore_emoji` (or removes it) on the first outbound event or after a `timeout_seconds` deadline; eliminates the 👀→🫡 manual two-step that currently appears in every voice message handler
