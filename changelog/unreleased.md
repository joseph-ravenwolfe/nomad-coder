...
- **Curated voice list with auto-rotation across sessions.** When `mcp-config.json` contains a non-empty `voices` array, each newly-created session is automatically assigned a voice via `pickRotationVoice(sid)` — `voices[(sid - 1) mod voices.length]`. With one voice, every session uses it; with N voices, sessions cycle through as they join. The Telegram `/voice` panel also renders this exact list instead of the dynamic provider fetch. New exports: `pickRotationVoice(sid)` and `setSessionVoiceForSid(sid, voice)` in `voice-state.ts`. Per-session overrides via `action(type: "profile/voice")` continue to take precedence over the rotation.
